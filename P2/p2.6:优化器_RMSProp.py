...
#   （4）RMSProp (含momentum的SGD)，在SGD基础上增加二阶动量：用t-1时刻的二阶动量与当前时刻的梯度平方进行指数滑动平均。可以对模型中的每个参数分配自适应的学习率。
#   m_{t} =  g_{t}        V_{t} = \beta * V_{t-1} + (1- \beta) * g_{t}^2
#   \eta_{t} = lr * m_{t}/\sqrt{V_{t}} =  lr * g_{t} / （\beta * V_{t-1} + (1- \beta) * g_{t}^2）
#   w_{t+1} = w_{t} - \eta_{t} =  w_{t} -  lr * m_{t}/ \sqrt{V_{t}} = w_{t} -  lr * g_{t} / （\beta * V_{t-1} + (1- \beta) * g_{t}^2


#   参数更新
#   RMSProp (含二阶momentum的SGD)
#   beta = 0.9
#   v_w , v_b = 0,0
...
#   b1.assign_sub(lr * grad[1] / tf.sqrt(v_b))
