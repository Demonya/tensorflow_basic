...
#   （3）Adagrad (含momentum的SGD)，在SGD基础上增加二阶动量：从初试到当前梯度平方的累计和。可以对模型中的每个参数分配自适应的学习率。
#   m_{t} =  g_{t}       V_{t} = \sum_{\tau=1}^{t}{g_{\tau}^{2}}
#   \eta_{t} = lr * m_{t}/\sqrt{V_{t}} =  lr * g_{t}/(\sqrt{\sum_{\tau=1}^{t}{g_{\tau}^{2}}})
#   w_{t+1} = w_{t} - \eta_{t} =  w_{t} -  lr * m_{t}/ \sqrt{V_{t}} = w_{t} -  lr * g_{t}/(\sqrt{\sum_{\tau=1}^{t}{g_{\tau}^{2}}})


#   参数更新
#   Adagrad (含二阶momentum的SGD)
#
#   v_w , v_b = 0,0

#   v_w +=  tf.square(grad[0])
#   v_b +=  tf.square(grad[1])
#   w1.assign_sub(lr * grad[0] / tf.sqrt(v_w))
#   b1.assign_sub(lr * grad[0] / tf.sqrt(v_b))
